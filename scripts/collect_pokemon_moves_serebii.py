...
def get_table_type(header_text: str) -> Optional[str]:
    ...
    Returns:
        "level_up", "tm", "egg_moves", "special_moves", "move_tutor" or None if unknown
    """
...
def parse_level_up_moves(table: BeautifulSoup, is_legends: bool = False) -> List[Dict]:
    """
    ...
    For S&V, there's just: level
    
    Args:
        table: The dextable element
        is_legends: Whether this is a Legends table (has plus_level in cell)
        
    Returns:
        List of dicts with 'name', 'level' (and 'level_plus' for Legends moves)
    """
    moves = []
...
def parse_egg_or_special_moves(table: BeautifulSoup) -> List[Dict]:
    """
    ...
    Returns:
        List of dicts with 'name', 'type' (and possibly other fields)
    """
    moves = []
    rows = table.find_all('tr')
...
def parse_serebii_moves(soup: BeautifulSoup, base_name: str, variant_names: List[str]) -> Dict:
    """
    Parse move data from Serebii Gen 9 page using dextable structure.
    ...
    The page contains BOTH Legends Z-A and Scarlet/Violet tables in sequence.
    We identify boundaries and parse both games separately.
    
    Typical structure:
    - Tables with "Legends: Z-A" in header (Legends moves, TM, etc.)
    - Then tables with NO "Legends" prefix (S&V moves, TM, Egg, Special)
    ...
    Returns:
        Dict mapping pokemon name -> {"gen_9": {"Legends: Z-A": {...}, "Scarlet and Violet": {...}}, "url": "..."}
    """
    result = {}
    ...
    # Identify which tables are Legends and which are SV
    # Strategy: Find ALL tables with "Legends" in header - these are Legends tables
    # Everything after the last Legends table is S&V
    # Everything BEFORE the first Legends table might be Legends (needs adjacent Legends header)
    
    legends_tables = []
    sv_tables = []
    
    # Find the index of the last Legends table
    last_legends_idx = -1
    ...
    for idx, table in enumerate(all_tables):
        ...
        # Check if this is a move table
        is_move_table = any(x in header_lower for x in [
            'level up', 'level-up', 'technical machine', ' tm ', 
            'egg', 'special', 'move tutor', 'tutor'
        ])
        
        if not is_move_table:
            # Skip non-move tables
            continue
        
        # Partition: tables before or at last_legends_idx are Legends,
        # tables after are S&V
    ...
    if legends_tables:
        games_data['Legends: Z-A'] = _parse_game_section(
            legends_tables, variant_names, base_name, is_legends=True
        )
    
    if sv_tables:
        games_data['Scarlet and Violet'] = _parse_game_section(
            sv_tables, variant_names, base_name, is_legends=False
        )
    ...
    for variant_name in variant_names:
        ...
        for game_name, variant_data in games_data.items():
            if variant_name in variant_data:
                result[variant_name]['gen_9'][game_name] = variant_data[variant_name]
    
    return result


def _parse_game_section(tables: List, variant_names: List[str], base_name: str, is_legends: bool = False) -> Dict:
    """
    Parse all move tables in a game section (Legends or SV).
    
    Args:
        tables: List of dextable elements to parse
        variant_names: List of variant names for this Pokemon
        base_name: Base Pokemon name
        is_legends: Whether this is Legends section (skip Legends-specific headers)
    ...
    for table in tables:
        ...
        # Skip Legends-prefixed headers if we're in SV section
        if not is_legends and 'legends' in header_lower:
            continue
        ...
        if table_type == "level_up":
            ...
            moves = parse_level_up_moves(table, is_legends=is_legends)
            ...
            # Egg moves - form-specific
            pokemon_name = match_form_name(header_text, variant_names)
            if not pokemon_name:
                pokemon_name = variant_names[0] if variant_names else base_name
            
            moves = parse_egg_or_special_moves(table)
            result[pokemon_name]['egg_moves'] = moves
        
        elif table_type == "special_moves" or table_type == "move_tutor":
            # Special/tutor moves - form-specific
            pokemon_name = match_form_name(header_text, variant_names)
            if not pokemon_name:
                pokemon_name = variant_names[0] if variant_names else base_name
            
            moves = parse_egg_or_special_moves(table)
            category_key = table_type
            if category_key not in result[pokemon_name]:
                result[pokemon_name][category_key] = []
            result[pokemon_name][category_key].extend(moves)
...
def main():
    ...
    for i, (base_name, variant_names) in enumerate(pokemon_by_base.items(), 1):
        ...
        try:
            ...
            moves = parse_serebii_moves(soup, base_name, variant_names)
