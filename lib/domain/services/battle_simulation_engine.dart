...
class BattleSimulationEngine {
    ...
    for (final turnAction in switchActions) {
      ...
      if (switchedOutPokemon != null) {
        ...
        if (slotIndex >= 0) {
          ...
          final opponent = _getFirstActiveOpponent(switchedInPokemon, finalStates,
  ...
  List<SimulationEvent> _executeMove({
    ...
    for (final defender in defenders) {
      ...
      if (damageResult.isDamageBlocked) {
        var message =
            '${defender.pokemonName} was protected!';
        events.add(SimulationEvent(
          message: message,
  ...
  List<BattlePokemon> _findDefenders(
    ...
    if (targetName == 'all-opposing') {
      ...
      final allField = [...currentFieldTeam1, ...currentFieldTeam2];
      return allField.where((p) => p.originalName != attacker.originalName && p.currentHp > 0).toList();
    }
  ...
  bool _isProtected(
    ...
    final attackerIsTeam1 = 
      currentFieldTeam1.any((p) => p.originalName == attacker.originalName);
    final attackerTeam = attackerIsTeam1 ? currentFieldTeam1 : currentFieldTeam2;
    final defenderIsTeam1 =
      currentFieldTeam1.any((p) => p.originalName == defender.originalName);
    final defenderTeam = defenderIsTeam1 ? currentFieldTeam1 : currentFieldTeam2;
    final allyAttackActions = [for (var ally in defenderTeam) ally.queuedAction].whereType<AttackAction>().toList();
    final allyAttackNames = [for (var action in allyAttackActions) action.moveName].toList();
    final allyDefenders = defenders.where((p) => defenderTeam.contains(p));
    if (allyAttackNames.contains("Wide Guard") && attackerTeam != defenderTeam && allyDefenders.length > 1) {
      return true;
    }
    ...
    if (move.priority > 0 && move.priority < 4 && defenderAction.moveName == "Quick Guard") {
    ...
    if (move.category.toLowerCase() == "status" && (defenderAction.moveName == "Crafty Shield" || allyAttackNames.contains("Crafty Shield"))) {
      return true;
    }
